...
export default async function PejabatPage({ params }: { params: Promise<{ nama: string }> }) {
  ...
  const data = await getPejabat(decodeURIComponent(nama))
  if (!data) notFound()
    ...
    <>
      ...
      <main className="report-shell mx-auto max-w-4xl px-6 py-8">
        <div className="no-print mb-5 flex items-center justify-between gap-3">
          <Link href="/" className="inline-flex items-center gap-1.5 rounded-full bg-white px-4 py-2 text-sm font-medium text-slate-600 shadow-soft ring-1 ring-slate-900/[0.06] transition-all hover:text-navy hover:shadow-card">← Kembali</Link>
