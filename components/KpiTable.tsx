...
// Tabel KPI Kolegial read-only, dikelompokkan per perspektif, dengan total skor.
export function KpiTable({ items, totalTarget = 100 }: { items: KpiItem[]; totalTarget?: number }) {
  const totalBobot = items.reduce((a, b) => a + (b.bobot ?? 0), 0)
  const totalSkor = items.reduce((a, b) => a + (b.skor ?? 0), 0)

  // Kelompokkan menjaga urutan kemunculan perspektif.
  const groups: { perspektif: string; rows: KpiItem[] }[] = []
  ...
  return (
    <div className="overflow-x-auto">
      <table className="w-full table-fixed border-collapse text-[12px]">
        ...
        <thead>
          <tr className="border-b border-slate-900/[0.08] text-left text-[10px] font-semibold uppercase tracking-[0.06em] text-slate-400">
            ...
            <th className="py-2 pr-2 font-semibold">Polaritas</th>
        ...
        <tbody>
            ...
            <Fragment key={g.perspektif}>
              ...
              {g.rows.map((it) => (
                <tr key={it.id} className="border-b border-slate-900/[0.05] align-top">
                  ...
                  <td className="py-2 pr-2 text-slate-500">{it.polaritas ?? "—"}</td>
          ...
          <tr className="border-t-2 border-slate-200">
            <td className="py-2 pr-2 font-bold text-navy" colSpan={5}>Total</td>
      ...
      <p className="mt-2 text-[11px] text-slate-400">
        Total skor <span className="data font-semibold text-navy">{fmtNum(totalSkor)}</span> dari target{" "}
        <span className="data">{totalTarget}</span>.
      </p>
    </div>
  )
}
